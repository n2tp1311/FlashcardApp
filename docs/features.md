...
- Forced retype after Learning/Hard in Flashcard & Write mode: grading a card Learning or Hard (the two grades signaling it didn't stick) now requires correctly retyping the answer once, on the back face, before the session auto-advances — a reinforcement drill layered on top of the existing self-check input, only for the two grades that need it. Prev/Next/Shuffle/Edit/Delete/re-grading/swipe-to-grade/card-flip are locked for the drill's duration; Exit stays reachable as an escape hatch. Matching is trim/case/whitespace-insensitive; empty answer text skips the drill entirely
