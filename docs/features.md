...
- Streak and all-time Study Time (total/avg/min/max per study day) are combined into one larger "hero" card at the top of the summary board on both Home and Dashboard — same flat surface/border styling as the plain classes/lessons/cards/sessions/attempts tiles next to it (just bigger), not a colored accent; avg/min/max render as small nested stat-card chips inside it. All-time, unlike the windowed Study Time chart further down the Dashboard — matches the lifetime scope of the streak and the other summary counts it sits next to. Days with attempts but no tracked duration (made before `duration_ms` shipped) are excluded from avg/min/max rather than counted as a 0-minute day; a tooltip on the avg/min/max row clarifies they're based on the count of tracked days, not the full streak length
