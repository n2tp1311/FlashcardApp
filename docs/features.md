...
- Manual difficulty grading: an optional ⚡ "Confident" button/key `4` alongside Still Learning/Hard/Know It sends `grade: "easy"` on the attempt, jumping the SRS step by +2 instead of the default +1 (labeled "Confident" rather than "Easy" to avoid colliding with the unrelated Easy/Medium/Hard difficulty-tier badge system)
- "Hard" grading button/key `2` (between Still Learning and Know It): reports a card as correctly recalled but effortful, sending `grade: "hard"`. Drops `srs_step` back by 1 (floored at 0) instead of resetting to 0 like an incorrect answer — a successful-but-slow recall is a desirable-difficulty signal, not a lapse, per learning-science research on spaced repetition. Keyboard shortcuts for all 4 grading buttons were renumbered to match their left-to-right screen order (1/2/3/4 = Learning/Hard/Know It/Confident)
