...
- Optional "Type answer before flipping" (Preferences toggle, off by default): shows a text input on the card front, auto-focused on every new card so typing needs no click first — type a guess, flip (click or Enter) to see it echoed as "Your answer: ..." above the back content for self-comparison. Purely cosmetic; grading is untouched, still driven only by the 4 mark buttons. Not the same as the removed Recall mode (a separate self-grading study mode) — this is opt-in, lives inside existing Flashcard mode, and never writes to attempts/SRS
