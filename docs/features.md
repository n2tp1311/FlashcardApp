...
- The whole summary board (both Home and Dashboard) is now a single "hero" card: Day Streak and all-time Study Time headline it by default, with Avg/Min/Max per study day and the Classes/Lessons/Cards/Sessions/Attempts counts folded in below as small nested stat-card chips — same flat surface/border styling as a plain `.stat-card`, just bigger, not a colored accent. Study Time is all-time, unlike the windowed Study Time chart further down the Dashboard — matches the lifetime scope of the streak and counts it's grouped with. Days with attempts but no tracked duration (made before `duration_ms` shipped) are excluded from avg/min/max rather than counted as a 0-minute day; each of those three chips has a tooltip clarifying they're based on the count of tracked days, not the full streak length. The chip grid uses `auto-fill` on desktop and a forced 3-column layout on mobile (matching the rest of the app's mobile grid pattern) so chips get fixed cell widths instead of shrinking/truncating
