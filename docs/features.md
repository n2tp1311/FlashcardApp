...
- Streak and all-time Study Time (total/avg/min/max per study day) are combined into one gradient "hero" card at the top of the summary board on both Home and Dashboard, visually set apart from the plain classes/lessons/cards/sessions/attempts count tiles. All-time, unlike the windowed Study Time chart further down the Dashboard — matches the lifetime scope of the streak and the other summary counts it sits next to. Days with attempts but no tracked duration (made before `duration_ms` shipped) are excluded from avg/min/max rather than counted as a 0-minute day
