...
- Class (course) tags: comma-separated free text on each class (normalized — trimmed, lowercased, deduped, capped at 10), shown as chips on class cards (Grid + List, below the meta line); a tag filter/slicer on Home (separate from, and composes with, the existing Level filter) narrows the class list by tag; Global Search also matches class tags, not just names. Works in both server and local/offline mode. Tags survive export/import round-trips and class-share cloning. (Tags briefly lived at the lesson level first — moved to classes per feedback that course-level tagging matters more)
...
- Spaced repetition scheduler: FSRS (Free Spaced Repetition Scheduler, `ts-fsrs`) — each card tracks its own stability/difficulty/state rather than sharing one fixed interval ladder with every other card. Quiz-mode correct answers are rated Hard rather than Good, so recognition earns a shorter next interval than an equivalent Flashcard-mode recall, by construction of the algorithm — the direct successor to the old step-ceiling clamp. Existing per-card progress was preserved via a one-time migration (see `docs/decisions.md`)
- "Needs Recall" filter on Study Setup (right after "Due Only"): surfaces due cards whose most recent correct answer came from quiz recognition, never from actively recalling them in Flashcard/Recall mode, so cards only weakly confirmed can be proactively cleared in one batch rather than discovered piecemeal during Quiz sessions. Selecting it auto-switches Mode to Flashcard (still overridable) since quiz mode can't confirm active recall by definition
- Flashcard interval preview: the 4 grading buttons show how long until the card comes back if graded that way, precomputed server-side from the live FSRS scheduler (not duplicated client-side) so the preview always matches what actually happens
- Memory Interval Distribution chart (Dashboard, deliberately not named after the underlying algorithm — end users don't need to know it's FSRS): buckets cards by FSRS state (Learning/Relearning get their own bucket) and interval range, using the same perceptual boundaries (10m/1h/4h/.../1yr) the old step-based chart used. Respects the 7/30/60/90-day period pill (scoped to cards last reviewed in that window) like every other chart in its section — initially shipped ignoring the pill entirely, fixed same day, see `docs/decisions.md`
...
- `GET /health`: trivial DB-connectivity check, rate-limited (60/min/IP). Wired into `railway.toml`'s `healthcheckPath` — gates new deploys (Railway won't cut traffic to a deploy that fails it) but does **not** monitor an already-live instance; see `docs/decisions.md` for why that distinction mattered
- Event-loop watchdog (`server/middleware/watchdog.js`): since `node-sqlite3-wasm` is synchronous, one heavy DB call blocks every concurrent request on the process. The watchdog force-exits if the event loop is unresponsive past a threshold (default 20s, `WATCHDOG_MAX_LAG_MS`), turning a hang into a crash that `railway.toml`'s `restartPolicyType` actually reacts to. Uses a monotonic clock (`process.hrtime`), not wall-clock time, so a host clock jump can't falsely trigger it
...
- Fixed: moving to a new card while the previous one was still flipped (common after grading + auto-advance) animated the un-flip with the new card's content already loaded, briefly revealing its answer mid-rotation; the reset is now instant, not animated
- "Flashcard & Write" study mode (Study Setup's 3rd Mode pill, alongside Flashcards and Quiz): shows a text input on the card front, auto-focused on every new card so typing needs no click first — type a guess, flip (click or Enter) to see it echoed as "Your answer: ..." above the back content for self-comparison. Purely cosmetic; grading is untouched, still driven only by the 4 mark buttons — same cards, same SRS behavior as plain Flashcard mode, just this one difference. Not the same as the removed Recall mode (a separate self-grading study mode) — never writes to attempts/SRS beyond the normal Flashcard grading. Was a Preferences toggle (off by default, hidden unless discovered) before becoming its own mode — see `docs/decisions.md`
- Forced retype after Learning/Hard in Flashcard & Write mode: grading a card Learning or Hard (the two grades signaling it didn't stick) now requires correctly retyping the answer once, on the back face, before the session auto-advances — a reinforcement drill layered on top of the existing self-check input, only for the two grades that need it. Prev/Next/Shuffle/Edit/Delete/re-grading/swipe-to-grade/card-flip are locked for the drill's duration; Exit stays reachable as an escape hatch. Matching tolerates one typo (a dropped/added/substituted/transposed letter, or a missing accent) via an edit-distance check, not just trim/case/whitespace normalization — an exact match shows "Correct!", a within-tolerance one shows "Close enough!"; empty answer text skips the drill entirely
- LaTeX-bearing answers get a manual confirm instead of the typed check: since `state.studyBackText` is the raw `$...$` source (not what's rendered), typing it back blind isn't a fair test — an "I recalled it — Continue" button swaps in, still gating the advance the same way a correct retype would, keyboard-activatable (Enter/Space) and auto-focused. Plain Flashcard mode (write-mode off) is unaffected — Learning/Hard there still auto-advances immediately, as before
- Manual difficulty grading: an optional ⚡ "Confident" button/key `4` alongside Still Learning/Hard/Know It sends `grade: "easy"` on the attempt (rated FSRS "Easy" — a longer next interval than an ungraded correct answer) (labeled "Confident" rather than "Easy" to avoid colliding with the unrelated Easy/Medium/Hard difficulty-tier badge system)
- "Hard" grading button/key `2` (between Still Learning and Know It): reports a card as correctly recalled but effortful, sending `grade: "hard"` (rated FSRS "Hard" — a weaker but still successful review, not a lapse) instead of resetting like an incorrect answer — a desirable-difficulty signal, per learning-science research on spaced repetition. Keyboard shortcuts for all 4 grading buttons were renumbered to match their left-to-right screen order (1/2/3/4 = Learning/Hard/Know It/Confident)
- The `grade` value submitted with every attempt (previously computed into the SRS step and discarded) is now persisted on the `attempts` row, enabling future grade-based analytics or scheduler work
- Session summary screen: grading the last card auto-shows the summary (same auto-advance timer that moves between cards; no manual "Finish" click needed) with total cards graded, a New vs. Review split, and a per-status breakdown (Learning/Hard/Know It/Confident, colored to match the grading buttons); notes how many cards were skipped (navigated past without grading) if any were. A "Review" button goes back to the flashcard screen (same card) for Prev/Next browsing, since auto-finish removes the pause to look the last card over; "Back to Lesson" leaves for good. The Finish button remains as a fallback for a last card reached without grading it. Quiz mode already has its own Results screen (score/percentage/grade) and wasn't touched
...
- Saveable Study Setup presets: "+ Save current as preset" captures the current Count/Filter/Mode/Order pill selection under a name; saved presets show as chips at the top of Study Setup — click a chip to reapply all 4 selections at once (hints and live match-count update accordingly), or its × to delete. Persisted server-side per-user (same preferences blob as dark mode/font scale), so presets survive reload; server mode only. The first 9 chips also show a small numbered badge — pressing that digit key applies the preset without a click (see Keyboard-Only Mode)
- Max reviews per day (Preferences): caps how many due cards get queued into a session, prioritizing the most-overdue cards first. Applies to Study Setup's Due Only/Needs Recall filters and the lesson screen's "Review N due" quick-start button — due-count badges everywhere else (Dashboard, per-class, per-lesson) stay uncapped/true, matching Anki's own "limits affect sessions, not totals" behavior. "Reviews done today" approximates to any graded attempt today (no new-vs-review distinction exists in the data model); a cap of 0 is a real "study nothing today" setting, distinct from an empty field (no limit); server mode only
...
- Grading buttons preview their resulting interval (e.g. "Know It · 4h"): precomputed server-side from the live FSRS scheduler (`server/fsrs.js:previewIntervals`, attached to every card as `fsrs_preview_*`) so it can never drift from what actually happens; suppressed for a not-yet-due card since grading it leaves the schedule untouched regardless of grade (mirrors the server's early-return in `attempts.js`); the local card's `srs_due_at` is updated from the attempt response so re-grading the same card via Prev reflects the new state
...
- **New-card-today estimate** (`GET /api/stats/new-card-estimate`, server mode only): recommends how many never-studied cards to introduce today, based on the user's historical new-card pace (average cards-first-attempted per active day) adjusted by their accuracy over the last 7 days (boosted for ≥85%, held steady 70–84%, cut to 70%/40% below that), clamped to however many un-introduced cards actually remain. A distinct concept from the existing "Max reviews per day" preference — that caps due-review load, this estimates new-card intake; kept as a separate endpoint/number so the two daily budgets don't get merged in the UI. Falls back to a flat default (10, or fewer if fewer cards remain) with different copy for accounts with under 3 days of new-card history, so a fresh account doesn't get shown a fake-precise personalized number. Originally also shown as its own callout on Study Setup; removed from there per feedback and now surfaced only via the summary board's customizable metrics (see below) — the endpoint and its messaging are unchanged, just relocated
...
- Voice selection is automatic (not user-configurable) and language-aware: the text actually being spoken is scanned for Vietnamese-exclusive diacritics (đ, ơ/ư, ă, and the stacked-tone range đ ẹ ị ọ etc.) to pick a Vietnamese or English voice pool, falling back to the app's UI language when the text has no detectable script signal (e.g. a Vietnamese sentence using only shared-with-French accents). Within the chosen pool, ranks Google voices first, then other quality-tier engines (Enhanced/Premium/Neural on Apple, Wavenet/Studio on Android, "Online (Natural)" on Edge), then any voice in the exact region, then whatever's available — falling back to English (never silence) if no voice exists for the detected language at all. Voices are cached per-language and refreshed together via the `voiceschanged` event (with synchronous init for Safari, which never fires it)
...
- Study Setup: `1`–`9` select a preset in its displayed order (a small numbered badge on each preset chip mirrors the key), `Enter` start studying, `Esc`/`⌫` back — completes the keyboard-only path (press `S` on a lesson to open setup, pick a preset by number, then `Enter` to begin, no mouse needed)
...
- **90-day study heatmap** — GitHub-style calendar; cells colored by daily attempt count (0 / 1–5 / 6–15 / 16+); month labels; UTC date keys match server's SQLite `date('unixepoch')` dates. Hovering a cell shows a friendly date, formatted study time, and attempt count together (e.g. "Jul 18: 5m studied (1 attempt(s))") rather than just the attempt count — a day with attempts but no tracked duration shows "0m" rather than breaking
- Heatmap rows are labeled Sun–Sat down the left side (sticky when the grid scrolls horizontally) so a day-of-week study pattern reads as a bright/dark horizontal band instead of requiring the reader to count rows
...
- **New Cards Trend** — same week-bucketed bar chart as the rolling trend above, but counts cards by the week of their *first-ever* attempt (a card's "new to you" date) rather than total attempts, so it reads as introduction pace over time rather than study volume. Derived entirely from existing `attempts` history — no new storage — reusing the same first-attempt-per-card query that powers the New Cards Today estimate, just bucketed by week instead of averaged down to one number
...
- **Accuracy by source** — Overall Accuracy gains secondary pills splitting Quiz (objective) vs Flashcard (self-rated) accuracy, since blending the two obscures how reliable the headline number actually is. Always exactly Quiz + Flashcard — legacy `source='recall'` attempts still count toward the overall total but are never broken out as their own pill
...
- **Future Due forecast** — "Upcoming Reviews (Next 14 Days)" chart next to Memory Interval Distribution: one bar per day (today + next 14) showing how many SRS-tracked cards become due that day. `GET /api/stats/future-due`, fixed 14-day window (not affected by the 7/30/60/90-day period pills — those are backward-looking past-attempts stats, this is forward-looking). Reflects currently-scheduled due dates only, not a simulation of what happens after each card is reviewed again. Addresses one specific piece of a broader "shallow analytics vs Anki" audit finding — retention-over-time curve, per-card history chart, ease/button distribution, and review-time stats remain unaddressed
...
- The whole summary board (both Home and Dashboard) is now a single "hero" card: Day Streak and Study Time headline it by default, with Avg/Min/Max per study day and the Classes/Lessons/Cards/Sessions/Attempts counts folded in below as small nested stat-card chips — same flat surface/border styling as a plain `.stat-card`, just bigger, not a colored accent. Study Time (total, Avg, Min, and Max together) has its own day-window (All time/7/30/60/90 days) — a reversal of the original all-time-only design, see `docs/decisions.md`. Configured via the gear icon's "Customize metrics" modal ("Study Time window" row alongside the per-metric Hidden/Show/Highlight controls, applied on Save), shared state across both screens and persisted in `localStorage`; the active window shows as a suffix on the label itself (e.g. "Study Time(7)", "Avg/day(7)") rather than a separate control on the card. Days with attempts but no tracked duration (made before `duration_ms` shipped) are excluded from avg/min/max rather than counted as a 0-minute day; each chip has a tooltip clarifying they're based on the count of tracked days within the selected window, not the full streak length. The chip grid uses `auto-fill` on desktop and a forced 3-column layout on mobile (matching the rest of the app's mobile grid pattern) so chips get fixed cell widths instead of shrinking/truncating
- **Customizable metrics**: a gear icon on the hero card (top-right corner) opens a modal listing all 11 metrics (Day Streak, Study Time, New Cards estimate, Avg/Min/Max daily, Classes, Lessons, Cards, Sessions, Attempts), each with a 3-way Hidden/Show/Highlight pill selector. Highlighted metrics become headline stats at the top of the card (any number, not just the original 2 — the divider-separated row grows to fit); Show puts a metric in the chip grid below; Hidden removes it entirely. Persisted server-side per-user (same preferences blob as dark mode/max reviews per day), shared between Home and Dashboard since both render the same hero card from the same config. Saving re-renders the card in place from the last-fetched data (no network refetch). If every metric is set to Hidden, the card collapses to a single-line message with the gear icon still reachable so the board is never a dead end. New Cards reuses the same estimate/messaging from `GET /api/stats/new-card-estimate` (see above — this is now its only surface in the UI), fetched in parallel with an isolated `.catch` so a failure there can't blank the rest of the board; its card/chip label is a short "New Cards" (the modal keeps the full "New cards recommended today" text where there's room)
...
- The 4 chart cards in "Study Charts" (Heatmap, Weekly Trend, Memory Interval Distribution, Upcoming Reviews) pack into a 2-column CSS masonry layout instead of a uniform grid, so cards with very different natural heights (a 7-row calendar next to a 15-row bar list) don't leave ragged empty space or a blank grid cell. 2 columns, not 3 — with only 4 cards the browser can only choose between 3 possible break points, so 2 wide columns balance total height far better than 3 narrow ones (verified: ~80px of leftover height difference vs. ~350px at 3 columns). Study Time was removed from this section — it now lives in the summary board's hero card instead (all-time, rather than windowed by the period pills)
- Heatmap and Weekly Trend now show a "No study data yet." empty state instead of an empty grid/all-zero bar list when there are no attempts in the selected window, matching how SRS Distribution and Upcoming Reviews already behaved. Originally a masonry-balance fix (without it, a brand-new account's 2-column layout was as unbalanced as the 3-column problem it replaced, since 2 of the 4 charts collapsed to a one-line empty state and 2 didn't) but also a cleaner first impression for new accounts on its own
- Streak-reset countdown: a small caption under the Day Streak headline stat reading "Resets in Xh Ym", counting down to the next UTC midnight — matching the server's actual streak day boundary (`date(created_at, 'unixepoch')` in `server/routes/stats.js` has no local-time offset applied, so a streak day is a UTC calendar date, not the viewer's local date). A tooltip on the countdown spells this out explicitly since it can read as "wrong" to a non-UTC user (e.g. it flips over at 7am in Indochina Time). Refreshed every 60s via a single interval started once at script load; only rendered when Day Streak is in the highlighted/headline group, not when demoted to a small chip
...
- Fixed: lesson titles in Class Detail were ellipsizing mid-word even at realistic lengths — measured at 375px, the title box only got 123px of a 351px row because padding/gap/format-badge crowded out the space, not the deliberately-always-visible edit/delete icons (M13). Tightened mobile padding/gap and hid the format badge (still visible once the lesson is opened) to give the title the room back — the reported example now fits fully
- Fixed: sidebar class names had a `title` attribute for a tooltip that never fires on touch (no `:hover`), so a truncated name had no way to be read at all on mobile. Switched from ellipsis-truncate to wrap — the row's `min-height` (not a fixed height) lets it grow to 2 lines
