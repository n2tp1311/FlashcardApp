...
- SRS distribution chart (Dashboard): buckets cards by FSRS state (Learning/Relearning get their own bucket) and interval range, using the same perceptual boundaries (10m/1h/4h/.../1yr) the old step-based chart used
...
- **Future Due forecast** — "Upcoming Reviews (Next 14 Days)" chart next to SRS Interval Distribution: one bar per day (today + next 14) showing how many SRS-tracked cards become due that day. `GET /api/stats/future-due`, fixed 14-day window (not affected by the 7/30/60/90-day period pills — those are backward-looking past-attempts stats, this is forward-looking). Reflects currently-scheduled due dates only, not a simulation of what happens after each card is reviewed again. Addresses one specific piece of a broader "shallow analytics vs Anki" audit finding — retention-over-time curve, per-card history chart, ease/button distribution, and review-time stats remain unaddressed
