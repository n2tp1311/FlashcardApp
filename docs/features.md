...
- The whole summary board (both Home and Dashboard) is now a single "hero" card: Day Streak and total Study Time headline it by default (always all-time — matches the lifetime scope of the streak and counts it's grouped with), with Avg/Min/Max per study day and the Classes/Lessons/Cards/Sessions/Attempts counts folded in below as small nested stat-card chips — same flat surface/border styling as a plain `.stat-card`, just bigger, not a colored accent. Avg/Min/Max Study Time have their own day-window (All time/7/30/60/90 days) — a reversal of the original all-time-only design, see `docs/decisions.md`. Configured via the gear icon's "Customize metrics" modal (a row alongside the per-metric Hidden/Show/Highlight controls, applied on Save), shared state across both screens and persisted in `localStorage`; the active window shows as a suffix on the chip label itself (e.g. "Avg/day(7)") rather than a separate control on the card. Days with attempts but no tracked duration (made before `duration_ms` shipped) are excluded from avg/min/max rather than counted as a 0-minute day; each of those three chips has a tooltip clarifying they're based on the count of tracked days within the selected window, not the full streak length. The chip grid uses `auto-fill` on desktop and a forced 3-column layout on mobile (matching the rest of the app's mobile grid pattern) so chips get fixed cell widths instead of shrinking/truncating
