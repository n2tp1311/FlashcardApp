...
- Flashcard & Write mode only: grading a card ✗ Still learning or ↩ Hard forces a correct retype of the answer on the back face before auto-advance — nav/re-grading/swipe locked for the drill, Exit excepted; skipped entirely only for empty answers. LaTeX-bearing answers get a manual "I recalled it — Continue" confirm instead of the typed check (raw `$...$` source isn't fair to retype against the rendered formula), keyboard-activatable and still gating the advance. Matching tolerates at most 1 edit (insertion/deletion/substitution/adjacent transposition, or a missing accent — accents aren't specially folded, they just cost 1 edit like any other typo) once the answer is 4+ characters, via a fixed O(n) "one edit away" check over Unicode code points (no general edit-distance matrix, no length cap of any kind); shorter answers require an exact match
...
| Review screen | `#screen-upstream`, from the sidebar (badge = sum of `upstream_count` on `GET /api/classes`) or the home banner. `GET /api/upstream-changes` → `{count:{updated,deleted,total}, cards:[… data, prev_data, class_name, lesson_title …]}`, newest first, archived classes included. Word diff (token LCS, capped at 250k cells) unless a field contains LaTeX. `POST /api/cards/acknowledge-updates {cardIds}` (1–1000, all must be owned or 403). "Study updated cards" → `openSetup` over affected lessons, filter `updated`, `returnScreen: "upstream"` |
...
POST   /api/cards/acknowledge-updates    GET    /api/upstream-changes
...
| KnowledgeApp convert: mcq / true-false → term-def in place (server mode only) | Done | §5.13. `server/routes/integrations.js`, `cards.converted_from` in `server/db.js`, backup in `server/routes/exportImport.js`. Verified via a 21-check API script: a studied, known card keeps every schedule field and its attempts across conversion, same id, original kept, no flag, replay/invalid/400/cross-user, lessons flip only when fully converted, converted cards answerable, converted_from survives a backup round trip |
| Long-press to multi-select (touch) | Done | `onLongPress` in `client/app.js` on class grid cards, class list rows, lesson items and card items; CSS in `client/style.css` under `(hover: none) and (pointer: coarse)`. Verified with Playwright at 390×844 with touch in WebKit and Chromium, 36/36: enters select mode with the item selected and not opened, following tap adds, a moving press does not select, a click-less press does not swallow the next tap, a re-rendered card stays selected, counts and delete-selected update |
