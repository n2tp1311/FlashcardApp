...
| Read (reconcile) | `GET /classes` → classes with `lessons`, `cards`, `term_def_cards`, `linked_cards`. `GET /classes/:id/cards` → lessons (`format`, `position`, `card_count`) and term-def cards only (`position`, `term`, `def`, `external_id`, `upstream_change`, `studied` = `srs_due_at` set or `fsrs_reps > 0`, `known`). 404 for a class not owned. No pagination: a 1000-card class is well under a megabyte |
